...
function InputCard({ children, focused }: { children: React.ReactNode; focused: boolean }) {
  return (
    <div className={cn(
...
export default function ContactPage({ params }: PageProps) {
  ...
  const { timeSlot, customerName, customerPhone, setContact, addons, addAddon, removeAddon } = useBookingStore();
  ...
  function handleGoToPayment() {
    router.push(`/book/${slug}/steps/payment`);
  }

  const previewProducts = useMemo(() => products.slice(0, 4), [products]);

  const getQuantity = useCallback(
    (productId: string) => addons.find((a) => a.id === productId)?.quantity ?? 0,
    [addons]
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {/* ── Hero — full bleed ── */}
      {/* c-salmon token: #E8705A */}
      <div className="relative flex-shrink-0" style={{ height: '30%', backgroundColor: '#E8705A' }}>

        {/* Back button */}
        <button
          onClick={() => router.back()}
          aria-label="Kembali"
          className="absolute top-s32 left-s20 z-10 flex h-[40px] w-[40px] items-center justify-center rounded-full bg-white text-label shadow-sm transition-all hover:bg-white/90 active:scale-95"
        >
        ...
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 390 300" preserveAspectRatio="xMidYMid slice" fill="none">
          <circle cx="320" cy="40" r="120" fill="#F4856B" fillOpacity="0.5"/>
          <circle cx="60" cy="260" r="100" fill="#D4604A" fillOpacity="0.4"/>
          <path d="M100 280 C90 240 85 200 95 170 C98 158 108 152 118 158 L130 165 C136 142 150 138 158 148 L162 155 C168 134 182 132 188 144 L192 154 C198 136 212 136 216 150 L218 200 C218 220 210 255 200 280Z" fill="#FDDCB0"/>
          <path d="M290 280 C300 240 305 200 295 170 C292 158 282 152 272 158 L260 165 C254 142 240 138 232 148 L228 155 C222 134 208 132 202 144 L198 154 C196 136 184 136 182 150 L180 200 C180 220 188 255 198 280Z" fill="#FDDCB0"/>
          <rect x="128" y="80" width="134" height="148" rx="8" fill="white" fillOpacity="0.95"/>
          <rect x="136" y="88" width="118" height="104" rx="4" fill="#c8ede2"/>
          {/* Smile doodle */}
          <g transform="translate(195, 140)" stroke="#3a7a62" strokeLinecap="round" strokeLinejoin="round" fill="none">
            <circle cx="0" cy="0" r="28" strokeWidth="3" fill="white" fillOpacity="0.7"/>
      ...
      <div className="flex-1 overflow-y-auto bg-bg relative z-10">
        <div className="flex flex-col gap-s20 px-s20 pt-s32 pb-s32">
          ...
          <div>
            <p className="text-[13px] font-semibold text-label3 mb-[6px]">Hampir selesai! 🙏</p>
            <h1 className="text-[26px] font-bold text-label leading-tight">Maaf, perlu data<br/>dirimu dulu.</h1>
            <p className="text-[14px] text-label2 mt-[8px] leading-snug">Kami butuh ini buat ngabarin kamu sebelum sesi dimulai.</p>
          </div>
          ...
          <div className="flex flex-col gap-s8">
            <p className="text-[14px] font-medium text-label2">Nomor WhatsApp</p>
            <InputCard focused={phoneFocused}>
              <div className="flex items-center">
                <div className="flex items-center px-s16 py-[14px] border-r border-black/10">
                  <span className="text-[15px] font-medium text-label2 select-none">+62</span>
                </div>
                <input
                  type="tel"
                  value={phone}
            ...
            <p className="text-[12px] text-label3">Konfirmasi booking dikirim ke nomor ini.</p>
          </div>
      ...
      {showAddons && (
        <>
          ...
          />
          <div className="absolute bottom-0 left-0 right-0 z-50 flex flex-col rounded-t-[28px] bg-bg animate-sheetUp" style={{ maxHeight: '80%' }}>
            ...
            <div className="px-s20 pt-s8 pb-s16 flex-shrink-0">
              <h2 className="text-[20px] font-bold text-label">Mau tambah produk?</h2>
              <p className="text-[13px] text-label2 mt-[4px]">Pilih perawatan tambahan buat hasil yang lebih maksimal.</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-s20 pt-s12 pb-s4">
              <div className="flex flex-col gap-s8">
                {previewProducts.map((product) => {
                  ...
                  return (
                    <div key={product.id} className="flex items-center gap-s12 p-s16 bg-white rounded-r16 border border-sep">
                      <div className="text-3xl w-12 h-12 flex items-center justify-center bg-bg rounded-r12 flex-shrink-0">
                        {product.imageEmoji}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[14px] font-semibold text-label">{product.name}</p>
                        <p className="text-[12px] text-label2 mt-[2px]">{product.description}</p>
                        <p className="text-[13px] font-bold text-accent mt-[4px]">{formatRupiah(product.price)}</p>
                      </div>
                      <div className="flex items-center gap-s8 flex-shrink-0">
                        <button
                          ...
                          className={cn(
                            ...
                            qty === 0 ? 'border-sep text-label3 cursor-not-allowed' : 'border-accent text-accent hover:bg-accent-soft'
                          )}
                        >−</button>
                        <span className="text-[14px] font-semibold text-label w-4 text-center">{qty}</span>
                        <button
