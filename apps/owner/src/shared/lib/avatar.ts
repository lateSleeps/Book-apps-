 ...
 * for customer/user avatars based on their name.
 *
 * @usedBy
 * BookingRow, MobileBookingCard, DashboardSidebar (user footer)
 *
 * @notes
 * Color is deterministic — same name always produces same color.
 * Uses a hash function so colors are distributed evenly.
 */

/** Soft pastel background colors for avatars (Apple-inspired palette) */
